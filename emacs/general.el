...
(defun linkify (link)
  ...
  (let ((b (region-beginning))
	(e (region-end)))
    (goto-char b)
    (insert (concat "[[" link "]["))	; resets the region, so we have to cache points
    (goto-char (+ 4 (length link) e))	; account for offest of brackets & link
    (insert "]]")))

(defun shuffle-region-lines ()
  "Shuffle the lines in a region in random order"
  (interactive)
  (let ((l (region-lines)))
...
(defun shuffle-list (x)
  "Recursive function to shuffle a list x"
  (if (eq x '())
      '()
    (let ((e (nth (random (length x)) x)))
      (cons e (shuffle-list (remove e x))))))

(defun region-lines ()
  "Get lines in region as a list"
  (let ((l (buffer-substring-no-properties (region-beginning) (region-end))))
    (with-temp-buffer 
      (insert l)
      (split-string (buffer-string) "\n" t))))

(defun tokipona-lookup ()
  "Look up a toki pona word"
  (interactive)
  (let ((i (completing-read "Word: " tokipona-dict )))
    (message (cadr (assoc i tokipona-dict)))))

(setq tokipona-dict (list
		     ...
		     '("anu" "(connects phrases with “or” rather than “and”)")
		     '("awen" "enduring, kept, protected; to continue to ")
		     ...
		     '("insa" "centre, inside, content; organ, stomach")
		     '("jaki" "disgusting, unclean, toxic")
		     '("jan" "person, somebody")
		     '("jelo" "yellow, yellowish")
		     '("jo" "to have, carry, contain, hold")
		     '("kala" "sea creature")
		     '("kalama" "sound; to make a sound, utter")
		     '("kama" "arriving, coming, future; to become")
		     '("kasi" "plant, vegetation; grass, leaf")
		     '("ken" "to be able to, can, may; possible")
		     '("kepeken" "to use, with, by means of")
		     '("kijete" "santakalu Musteloids, such as racoon [joke word]")
		     '("kili" "fruit, vegetable")
		     '("kin" "also, too")
		     '("kipisi" "to cut, to divide; part, division")
		     '("kiwen" "hard object, metal, stone, rock")
		     '("ko" "clay, semi-solid, paste, powder")
		     '("kon" "air; spirit, essence; unseen agent")
		     '("ku" "to interact with")
		     '("kule" "color, colorful")
		     '("kulupu" "community, group, company, nation")
		     '("kute" "ear; to hear")
		     '("la" "(context marker)")
		     '("lape" "sleeping, resting")
		     '("laso" "blue, green")
		     '("lawa" "head, mind; ruler; to lead, to regulate")
		     '("leko" "square, block")
		     '("len" "cloth, fabric; cover, layer of privacy")
		     '("lete" "cool, cold; raw")
		     '("li" "(predicate marker)")
		     '("lili" "small, little; few, a bit; young")
		     '("linja" "long and flexible thing; string, cord, hair, thread")
		     '("lipu" "flat object; paper, card, document, website")
		     '("loje" "red, reddish")
		     ...
		     '("mije" "male")
		     '("moku" "to eat, to drink; food")
		     '("moli" "dead, dying")
		     '("monsi" "back, behind, rear")
		     '("monsuta" "fear; monster; scary")
		     '("mu" "(animal noise)")
		     '("mun" "night sky object")
		     '("musi" "entertaining, fun, recreational")
		     '("mute" "many, a lot; very; quantity")
		     '("namako" "spice, additional")
		     '("nanpa" "-th; number")
		     '("nasa" "strange, unusual; foolish; silly")
		     ...
		     '("olin" "to love, to respect")
		     '("ona" "third-person pronoun")
		     '("open" "to begin, to start; opening")
		     '("pakala" "broken, damaged; to mess up")
		     '("pali" "to do, to work on")
		     '("palisa" "long hard object; rod, stick")
		     '("pan" "grain, pasta, bread, rice")
		     '("pana" "to give, to emit, to send")
		     ...
		     '("selo" "outer form, outmost layer; skin, peel")
		     '("seme" "what? which?")
		     ...
		     '("suno" "light source, sun; bright")
		     '("supa" "horizontal surface, thing to put objects on")
		     '("suwi" "sweet; cute")
		     '("tan" "from, by, because")
		     '("taso" "but, however; only")
		     '("tawa" "going to; for; moving")
		     '("telo" "liquid, water, beverage")
		     '("tenpo" "time, duration, moment, period")
		     '("toki" "to say; language")
		     '("tomo" "indoor space; building, home")
...
(defun webp-to-png (image)
  "Elisp hook to run gimp and convert a given .webp file image to a png"
  (interactive "fImage: ")
  (let ((pngimage (replace-regexp-in-string "\.webp" "\.png" image)) (gimp-path "\"C:\\Program Files\\GIMP 2\\bin\\gimp-2.10.exe\""))
    (async-shell-command (concat gimp-path " -i -b \""
		     "(let ((image (file-webp-load 1 " "\\\"" image "\\\"" "\\\"" image "\\\"" "))) (file-png-save-defaults 1 (car image) (car (gimp-image-get-active-drawable (car image))) " "\\\"" pngimage "\\\" " "\\\"" pngimage "\\\"" ") (gimp-image-delete (car image)) (gimp-quit 0))\"") "*gimp-output*" "*gimp-error")))

;; Inspired by https://github.com/stumpwm/stumpwm-contrib/blob/master/util/searchengines/README.org but I'm on Windows
(defun google-region ()
    "Google the highlighted region"
    (interactive)
    (browse-url (concat "https://www.google.com/search?&q=" (replace-regexp-in-string "\s" "+" (buffer-substring (region-beginning) (region-end))))))

(setq bofh-excuses '("clock speed"
		     "solar flares"
...
(defun bofh-excuse ()
  "Pick a random excuse from the BOFH-approved list"
  (interactive)
  (message (nth (random (length bofh-excuses)) bofh-excuses)))
